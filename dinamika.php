...
// Dinamika davrini tanlash (7 kun, 30 kun, barchasi)
$davr = $_GET['davr'] ?? '7';
if ($davr == '30') {
    $interval = "30 DAY";
} elseif ($davr == 'all') {
...
// Ma'lumotlarni olish
if ($interval) {
    $stmt = $conn->prepare("SELECT * FROM hisobotlar WHERE tuman_id = ? AND sana >= DATE_SUB(?, INTERVAL $interval) ORDER BY sana DESC");
    $stmt->bind_param("is", $hudud_id, $sana);
} else {
    $stmt = $conn->prepare("SELECT * FROM hisobotlar WHERE tuman_id = ? ORDER BY sana DESC");
    $stmt->bind_param("i", $hudud_id);
}
$stmt->execute();
$hisobotlar = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Ma'lumotlarni teskari tartibda (eng eski -> eng yangi) grafik uchun
$grafik = array_reverse($hisobotlar);
...
// Grafik uchun maksimal qiymat
$max_soliq = 0;
$max_bozor = 0;
$max_qarz = 0;
foreach ($grafik as $g) {
    $max_soliq = max($max_soliq, $g['soliq_kunlik'] ?? 0);
    $max_bozor = max($max_bozor, $g['bozor_raqam'] ?? 0);
    $max_qarz = max($max_qarz, $g['und_jami'] ?? 0);
}
...
.btn-active { background: var(--blue); color: var(--white); border: none; }
...
.stat-card .value small { font-size: 13px; color: var(--text-gray); }
...
.chart-panel-header h3 { margin: 0; font-size: 18px; }
.chart-panel-header p { margin: 5px 0 0; font-size: 13px; color: var(--text-gray); }
...
<aside class="sidebar">
    ...
    <nav>
        ...
        <a href="dinamika.php" class="nav-link active">
...
<div class="main-content">
    <div class="dashboard-header">
        <div>
            <h2><?php echo matn('Динамика'); ?></h2>
            <p><?php echo matn('Ўзгаришлар таҳлили'); ?> — <?php echo htmlspecialchars($tuman_nomi); ?></p>
        </div>
    </div>

    <!-- Davr tanlash -->
    <div class="filter-panel">
        <a href="?davr=7" class="btn <?php echo $davr == '7' ? 'btn-active' : ''; ?>"><?php echo matn('7 кун'); ?></a>
        <a href="?davr=30" class="btn <?php echo $davr == '30' ? 'btn-active' : ''; ?>"><?php echo matn('30 кун'); ?></a>
        <a href="?davr=all" class="btn <?php echo $davr == 'all' ? 'btn-active' : ''; ?>"><?php echo matn('Барчаси'); ?></a>
    </div>
    ...
    <div class="stat-cards">
        <div class="stat-card">
            ...
            <div class="value"><?php echo number_format($jami['soliq_jami'], 2); ?> <small><?php echo matn('млн сўм'); ?></small></div>
            <div class="sub"><?php echo $jami['kunlar']; ?> <?php echo matn('кун давомида'); ?></div>
        </div>
        <div class="stat-card">
            <h3><?php echo matn('Кунлик тушум'); ?></h3>
            <div class="value"><?php echo number_format($jami['soliq_kunlik'], 2); ?> <small><?php echo matn('млн сўм'); ?></small></div>
            <div class="sub"><?php echo matn('Ўртача кунлик'); ?></div>
        </div>
        <div class="stat-card green::before">
            <h3><?php echo matn('Рақамлаштирилган бозор'); ?></h3>
            <div class="value"><?php echo number_format($jami['bozor_raqam']); ?></div>
            <div class="sub"><?php echo matn('Жами'); ?></div>
        </div>
        <div class="stat-card red::before">
            <h3><?php echo matn('Ундирилган қарз'); ?></h3>
            <div class="value"><?php echo number_format($jami['und_jami'], 2); ?> <small><?php echo matn('млн сўм'); ?></small></div>
            <div class="sub"><?php echo matn('Жами'); ?></div>
        </div>
    </div>

    <!-- Grafik 1: Kunlik soliq tushumi -->
    <div class="chart-panel">
        <div class="chart-panel-header">
            <h3><?php echo matn('Кунлик солиқ тушуми'); ?></h3>
            <p><?php echo matn('Ҳар бир кун учун (млн сўм)'); ?></p>
        </div>
        <div class="chart-panel-body">
            <?php if (empty($grafik)): ?>
                <div class="empty-state" style="text-align: center; padding: 50px; color: #6b7280;">
                    <b><?php echo matn('Маълумот йўқ'); ?></b>
                    <p><?php echo matn('Бу даврда ҳисобот киритилмаган.'); ?></p>
                </div>
            <?php else: ?>
                <div class="chart-container">
                    <?php foreach ($grafik as $g): ?>
                        <div class="chart-bar" style="height: <?php echo max(5, ($g['soliq_kunlik'] / $max_soliq) * 100); ?>%;" data-qiymat="<?php echo number_format($g['soliq_kunlik'], 2); ?> mln">
                            <span><?php echo date('d.m', strtotime($g['sana'])); ?></span>
                        </div>
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
    ...
    <div class="chart-panel">
        <div class="chart-panel-header">
            <h3><?php echo matn('Рақамлаштирилган бозорлар'); ?></h3>
            <p><?php echo matn('Ҳар бир кун учун'); ?></p>
        </div>
        <div class="chart-panel-body">
            <?php if (empty($grafik)): ?>
                <div class="empty-state" style="text-align: center; padding: 50px; color: #6b7280;">
            ...
            <?php else: ?>
                <div class="chart-container">
                    <?php foreach ($grafik as $g): ?>
                        <div class="chart-bar" style="height: <?php echo max(5, ($g['bozor_raqam'] / $max_bozor) * 100); ?>%; background: linear-gradient(to top, var(--green), #86efac);" data-qiymat="<?php echo number_format($g['bozor_raqam']); ?> та">
                            <span><?php echo date('d.m', strtotime($g['sana'])); ?></span>
                        </div>
                    <?php endforeach; ?>
                </div>
    ...
    <div class="chart-panel">
        <div class="chart-panel-header">
            <h3><?php echo matn('Ундирилган солиқ қарзи'); ?></h3>
            <p><?php echo matn('Ҳар бир кун учун (млн сўм)'); ?></p>
        </div>
        <div class="chart-panel-body">
            <?php if (empty($grafik)): ?>
            ...
            <?php else: ?>
                <div class="chart-container">
                    <?php foreach ($grafik as $g): ?>
                        <div class="chart-bar" style="height: <?php echo max(5, ($g['und_jami'] / $max_qarz) * 100); ?>%; background: linear-gradient(to top, var(--red), #fca5a5);" data-qiymat="<?php echo number_format($g['und_jami'], 2); ?> мln">
                            <span><?php echo date('d.m', strtotime($g['sana'])); ?></span>
                        </div>
                    <?php endforeach; ?>
                </div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th><?php echo matn('Тушум'); ?></th>
                    <th><?php echo matn('Кунлик'); ?></th>
            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php $i = 1; foreach ($hisobotlar as $h): ?>
                        <tr>
                            <td><?php echo $i++; ?></td>
